...
def _ensure_tenant_access(ctx: TenantContext, instance) -> None:
    """Reject cross-tenant access.

    The previous check ``if ctx.tenant and instance.tenant_id != ctx.tenant.id``
    silently skipped verification for tenantless users, letting them retrieve
    or mutate any workflow regardless of owning tenant. We now always require
    the workflow's ``tenant_id`` to match the caller's tenant (or both to be
    ``None`` for tenantless users).
    """
    caller_tenant_id = ctx.tenant.id if ctx.tenant else None
    if instance.tenant_id != caller_tenant_id:
...
def get_detail(workflow_id: str, db: Session = Depends(get_db), ctx: TenantContext = Depends(get_current_tenant)):
    ...
    _ensure_tenant_access(ctx, instance)
