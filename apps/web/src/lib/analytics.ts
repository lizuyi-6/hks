...
class Analytics {
  ...
  constructor(config: Partial<AnalyticsConfig> = {}) {
    ...
    // Only start timers and event handlers in browser.
  ...
  private async sendToServer(events: AnalyticsEvent[]): Promise<void> {
    ...
    try {
      ...
      // Re-queue the failed events (up to maxQueueSize to prevent unbounded growth).
  ...
  private setupPagehideFlush(): void {
    ...
    window.addEventListener("pagehide", () => {
      ...
      // Best-effort beacon send on page unload.
      try {
        navigator.sendBeacon(
          this.config.endpoint,
          JSON.stringify({ events: this.queue, session_id: this.sessionId })
        );
        this.queue = [];
      } catch {
        // sendBeacon unavailable; keepalive fetch was already set up above.
      }
    });
  }
