...
def normalize_contract_review_payload(payload) -> dict:
    """Coerce an LLM payload into the shape the合同审查 UI expects.

    The streaming path and the sync path both feed the same UI, which reads
    ``risks``, ``ip_clauses_found``, ``missing_clauses`` and ``overall_risk``
    directly off the envelope. When the LLM emits plain text (or partial JSON)
    we still want to surface a usable structure instead of crashing the UI.
    """
    if isinstance(payload, dict) and "risks" in payload:
        payload.setdefault("summary", "")
        payload.setdefault("ip_clauses_found", [])
        payload.setdefault("missing_clauses", [])
        payload.setdefault("overall_risk", "medium")
        return payload

    fallback_summary = ""
    if isinstance(payload, dict):
        fallback_summary = payload.get("analysis") or payload.get("summary") or ""
    elif isinstance(payload, str):
        fallback_summary = payload

    return {
        "summary": fallback_summary or "合同审查完成",
        "risks": [],
        "ip_clauses_found": [],
        "missing_clauses": [
            "建议补充知识产权归属条款",
...
class RealContractReviewAdapter(ContractReviewPort):
    ...
    def review(self, contract_text: str, trace_id: str):
        ...
        payload = normalize_contract_review_payload(llm_result.normalized_payload)
