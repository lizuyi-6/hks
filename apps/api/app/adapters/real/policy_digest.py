...
def normalize_policy_digest_payload(payload, industry: str) -> dict:
    """Coerce an LLM payload into the shape the政策速递 UI expects."""
    if isinstance(payload, dict) and "policies" in payload:
        payload.setdefault("industry", industry)
        payload.setdefault("key_changes", [])
    ...
    return {
        ...
        "compliance_notes": fallback_note or "建议定期查阅国家知识产权局官网获取最新政策。",
    }
...
class RealPolicyDigestAdapter(PolicyDigestPort):
    ...
    def digest(self, industry: str, trace_id: str):
        ...
        payload = normalize_policy_digest_payload(llm_result.normalized_payload, industry)
